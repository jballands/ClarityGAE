...
(function() {
  var apiCall, dataDeleteRecords, dataDetailRecord, dataInitButtons, dataLoadTable, dataMakeDetailView, dataMakeTableView, dataPageBack, dataPageButton, dataPageForward, dataParseTable, initialize, root;
  ...
  initialize = function() {
    ...
    return (apiCall('/static/data/data-config.json', {}, false, "GET")).done(function(data) {
      root.dataConfig = data;
      return dataInitButtons();
    });
  };

  apiCall = function(url, data, lazy, method) {
    if (data == null) {
      data = {};
    }
    if (lazy == null) {
  ...
  dataMakeDetailView = function(model, property, instance, cell, empty) {
    var button, editor, linkedID, name, options, request, type, url, value, _i, _len;
    if (empty == null) {
      empty = false;
    }
    name = property["name"];
      ...
      case 'modelList':
        ...
        for (_i = 0, _len = value.length; _i < _len; _i++) {
          ...
          switch (property["model"]) {
            case "ticket":
              request.done(function(data) {
                return button.text("Opened: " + data['opened']);
              });
              break;
          }
        }
